...
ARCANA_PIP = "git+ssh://git@github.com/australian-imaging-service/pydra2app.git"
...
class ClassResolver:
    ...
    def fromstr(cls, class_str, subpkg=None):
        ...
        if subpkg:
            full_mod_path = ".".join(("pydra2app", module_path, subpkg))
        else:
    ...
    def tostr(cls, klass, strip_prefix: bool = True):
        ...
        if strip_prefix and cls._get_subpkg(klass):
            ...
            if match := re.match(r"pydra2app\.(\w+)\." + subpkg, module_name):
...
def asdict(obj, omit: ty.Iterable[str] = (), required_modules: ty.Optional[set] = None):
    ...
    ----------
    obj
        The Pydra2App object to asdict. Must be defined using the attrs
        decorator
    omit: Iterable[str]
...
def fromdict(dct: dict, **kwargs):
    ...
    ----------
    dct : dict
        A dictionary containing a serialsed Pydra2App object such as a data store
    ...
    **kwargs : dict[str, Any]
    ...
    # try:
    #     pydra2app_version = dct["pkg_versions"]["pydra2app"]
    # except (TypeError, KeyError):
    #     pass
    #     else:
    #         if packaging.version.parse(pydra2app_version) < packaging.version.parse(MIN_SERIAL_VERSION):
    #             raise Pydra2AppVersionError(
    #                 f"Serialised version ('{pydra2app_version}' is too old to be "
    #                 f"read by this version of pydra2app ('{__version__}'), the minimum "
